...
Client.config = {
	...
	maxBatchSize: 1000, // max batch size for sets and deletes methods
	activeQueries: 0,
	maxQueueSize: -1,
...
;(function (nMemcached) {
	...
	privates.setters = function setters(type, validate, key, value, lifetime, callback, cas) {
		if (type == "sets" && key.length != value.length) {
			return privates.errorResponse(new Error("Number of keys and values don't match"), callback)
		}

		var fullkey
		if (toString.call(key) === "[object Array]") {
			var namespace = this.namespace
			fullkey = key.map(function (item) {
		...
		} else if (valuetype !== "string" && !Array.isArray(value)) {
			flag = FLAG_JSON
		...
		this.command(function settersCommand(noreply) {
			if (type == "sets") {
				...
				for (let i = 0; i < fullkey.length; i++) {
					command +=
						[type, fullkey[i], flag, lifetime, length[i]].join(" ") + (cas ? " " + cas : "") + (noreply ? NOREPLY : "") + LINEBREAK + value[i] + LINEBREAK
				}
				command = command.slice(0, command.length - LINEBREAK.length)
				return {
					key: fullkey,
					callback: callback,
					lifetime: lifetime,
	...
	memcached.sets = curry(undefined, privates.setters, "sets", [
		["key", Array],
		["value", Array],
		["lifetime", Number],
		["callback", Function],
	 ...
	 * @param {Array} keys
	 */
	memcached.dels = function dels(keys, callback) {
		var namespace = this.namespace
		var fullkey = keys.map(function (item) {
			return namespace + item
		})
		this.command(function deletesCommand(noreply) {
			var command = ""
			for (let i = 0; i < fullkey.length; i++) {
				command += "delete " + fullkey[i] + (noreply ? NOREPLY : "") + LINEBREAK
			}
			command = command.slice(0, command.length - LINEBREAK.length)
			return {
				key: fullkey,
	...
	memcached["deletes"] = memcached.dels
